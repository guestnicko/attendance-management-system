            ...
            <tr class="hover:bg-gray-50 transition-colors duration-200" id="{{ $student->id }}">
                ...
                <td class="px-6 py-4 text-center">
                    <div class="flex items-center justify-center gap-3">
                        ${editButton(`updateStudent(${JSON.stringify(student)}, "${student.s_image}")`, `open = true`)}
                        ${deleteButton(`deleteStudent(${JSON.stringify(student)})`)}
                    </div>
                </td>
            </tr>`;
...
function editButton(onClick, xClick) {
    const button = `
    <button
    onClick='${onClick}'
    x-on:click='${xClick}'
...
function deleteButton(onClick, xClick) {
    const deleteForm = document.getElementById("deleteStudent");
    const deleteFieldId = document.getElementById("s_id");
    const button = `
    <button
    onClick='${onClick}'
    x-on:click='${xClick}'
